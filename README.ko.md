...
- **키(API 토큰) 없이 기본 동작** — KBO, K리그, LCK/LPL/LEC/MSI/롤드컵/First Stand, MLB, NHL, ESPN의 축구·NFL·NBA·WNBA 중계, 그리고 1.1.0에서 더해진 테니스·포뮬러 1·UFC·크리켓·미국 대학농구까지 별도 설정 없이 바로.
...
| **Tennis** | ATP 투어, WTA 투어 🎾 | 단식과 복식. 스코어는 **획득 세트 수**입니다 — 6-2, 7-5 승리는 `2:0`으로 표시되고, 세트별 게임 스코어(`6-2, 7-5`)는 상태 텍스트에 들어갑니다. 복식 조는 `Hao-Ching Chan / Miyu Kato`처럼 표시되고 약칭은 `CH/KA` 형태입니다. 중계는 완료된 세트마다 한 줄, 그리고 경기 종료 한 줄이며 모두 확장이 직접 조립합니다. **포인트 단위 중계는 없습니다** — 상위 API에 테니스 플레이바이플레이 자체가 없습니다. | 키 불필요, **비공식** |
| **Motorsport** | 포뮬러 1 🏎 | 그랑프리 주말의 **세션마다 한 행** — 연습주행 1·2·3, 예선, 스프린트, 결승 — 이라서 금요일 연습주행이 결승으로 잘못 표시되는 일이 없습니다. 세션은 두 팀의 대결이 아니라 드라이버 22명 안팎의 필드이므로, 행에는 스코어 대신 대회명과 현재 선두가 표시됩니다(`Belgian Grand Prix — Practice 1 · 1위 VER`). 중계는 세션 시작·세션 종료, 그리고 상위 3명을 호명하는 결과 한 줄입니다. **순위 변동 중계는 없습니다.** | 키 불필요, **비공식** |
| **ESPN** | UFC 🥊 | 한 행이 경기 하나가 아니라 **대회(파이트 카드) 전체**입니다 — 상위 API가 하루치 대회를 이벤트 하나로 모델링하기 때문입니다. 행 이름은 그 카드의 **메인이벤트**에서 따오고, 중계는 카드에 오른 모든 경기를 경기당 결과 한 줄로 다룹니다. | 키 불필요, **비공식** |
| **ESPN** | 크리켓 🏏 | **스코어와 상태만.** 상위 요약 엔드포인트가 응답은 주지만 그 안에 플레이도 코멘터리도 없습니다. 그래서 팔로우한 크리켓 경기는 스코어와 상태를 계속 갱신하지만 중계 채널은 비어 있습니다. | 키 불필요, **비공식** |
| **ESPN** | 미국 대학미식축구 🏈, 미국 남자 대학농구 🏀 | 플레이바이플레이 피드, 득점 플레이 강조 — NFL·NBA와 같은 방식입니다. 대학미식축구는 **기본으로 켜져 있지 않습니다**([대학미식축구 켜기](#대학미식축구-켜기) 참고). | 키 불필요, **비공식** |
| **PandaScore** | LoL, CS2, 도타 2, 발로란트 🎮 | **스코어 티커만**(맵 결과) — 라운드/킬 단위 코멘터리는 없음 | 무료 API 토큰 필요, 비공식 |

`vibeStealth.leagues.enabled`에 쓰는 리그 id는 `provider:league` 형식입니다 — 예: `naver:kbo`, `lolesports:lck`, `mlb:mlb`, `nhl:nhl`, `espn:eng.1`, `espn:ufc`, `espn:cricket`, `espn-tennis:atp`, `espn-racing:f1`, `pandascore:lol`.

### 대학미식축구 켜기

`espn:college-football`은 완전히 지원하지만 기본값은 **꺼짐**입니다. 토요일 하루에만 99경기 안팎이 잡히는데 트리는 경기당 한 행을 자르지 않고 그리므로, 켜 두면 다른 리그가 전부 묻힙니다.

켜려면 설정의 `vibeStealth.leagues.enabled`에 리그 id를 추가하세요:

```json
"vibeStealth.leagues.enabled": ["espn:college-football"]
```

이 설정은 추가 목록이 아니라 **전체 허용 목록**입니다. 값이 하나라도 들어가는 순간, 여기 적은 id만 트리에 나타납니다. 그러니 함께 보고 싶은 리그도 같이 적어야 합니다 — 예: `["espn:college-football", "mlb:mlb", "naver:kbo"]`.
...
- **실시간 상황 행이 있는 종목은 야구·축구·LoL뿐입니다.** 테니스, 모터스포츠, UFC, 크리켓, 미식축구, 농구 경기는 펼칠 수 없고 아래에 아무것도 없습니다. 테니스와 모터스포츠가 보여줄 수 있는 것은 경기 행 자체에 있습니다 — 테니스는 상태 텍스트의 세트별 게임 스코어(`6-2, 7-5`), 모터스포츠는 행 라벨의 현재 선두(`1위 VER`)이며 둘 다 폴링마다 다시 그려집니다.
...
**🎾 테니스, 🏎 모터스포츠, 🥊 UFC, 🏏 크리켓은 이 설정을 무시합니다.** 넷 다 상위 API에 끌어올릴 플레이바이플레이 피드가 아예 없어서, `detailed`로 바꿔도 `summary`와 완전히 같은 라인만 나옵니다 — 테니스는 완료된 세트와 경기 결과, 모터스포츠는 세션 경계와 시상대, UFC는 경기당 결과 한 줄, 크리켓은 아무것도 없습니다.
...
**테니스와 모터스포츠 행에는 아무 이미지도 붙지 않습니다.** 단체 종목이 엠블럼을 다는 자리에서 이 행들은 기본 아이콘으로 돌아갑니다 — 단식·복식·레이스 세션 모두 마찬가지입니다. 상위 API에 선수 프로필 사진과 드라이버 사진이 있기는 하지만, 위 허용 목록에 없는 호스트에서 제공되므로 확장이 내려받지 않습니다.
...
- **구조화된 필드에서 확장이 직접 조립하는 텍스트는 현지화됩니다.** 투구 라인, NHL 이벤트, LoL 킬 피드, 트리의 모든 상황 행, 모든 시스템 라인이 선택한 언어로 나옵니다. 테니스·모터스포츠·UFC는 통째로 이쪽에 속합니다. 상위 API에 문장 자체가 없어서 **모든** 중계 라인을 확장이 구조화된 스코어에서 조립하므로, 세트 라인·경기 결과·세션 경계·시상대·경기 결과가 전부 한국어로 나옵니다.
- **API가 프로즈로 내려주는 텍스트는 그대로 통과시키며, 절대 기계번역하지 않습니다.** MLB 타석 설명과 ESPN 축구 코멘터리는 영어 그대로입니다.
- **네이버의 KBO/K리그 중계는 애초에 한국어**입니다.
- **고유명사는 피드가 적어 준 그대로 둡니다.** ESPN이 주는 대회명·경기명·팀명은 번역하지 않으며, 모터스포츠 행 이름 뒤에 붙는 세션 표기(`— Practice 1`)는 두 언어에서 같은 행으로 읽히도록 영어로 고정했습니다. 중계 채널에 흐르는 세션 라인은 한국어로 나옵니다(`연습주행 1 시작`).
...
| `vibeStealth.leagues.enabled` | array of string | `[]` | — | 트리에 표시할 리그를 `provider:league` id로 지정합니다(예: `espn:eng.1`, `mlb:mlb`, `nhl:nhl`, `espn-tennis:atp`, `espn-racing:f1`, `pandascore:lol`). 비워 두면 기본 제공 리그 세트를 쓰고, 값을 채우면 그 목록이 기본 세트를 대체하는 전체 허용 목록이 됩니다. `espn:college-football`은 지원하지만 기본 세트 밖이므로, 여기에 적어야 켜집니다. |
...
- **크리켓도 스코어 티커입니다.** 상위 요약 엔드포인트가 응답은 주지만 그 안에 플레이도 코멘터리도 없습니다. 그래서 팔로우한 크리켓 경기는 트리와 상태 표시줄에서 스코어·상황을 계속 갱신하면서도 중계 채널은 비어 있습니다. 무언가를 걸러내는 것이 아니라, 중계할 내용 자체가 없습니다.
- **UFC 한 행은 경기 하나가 아니라 대회 전체입니다.** 상위 API가 하루치 대회를 이벤트 하나로 모델링하므로, 행 이름은 그 카드의 메인이벤트에서 따오고, 중계는 카드에 오른 모든 경기를 경기당 결과 한 줄로 다룹니다.
- **테니스에 포인트 단위 중계는 없습니다.** 상위 API에 플레이바이플레이가 없어서, 중계는 완료된 세트마다 한 줄과 경기 결과 한 줄뿐입니다 — 게임 단위도, 포인트 단위도 아닙니다. 세트별 게임 스코어는 경기 행의 상태 텍스트에 있습니다.
- **모터스포츠는 순위 변동을 알리지 않습니다.** "해밀턴 3위로 상승" 같은 라인은 나오지 않습니다. 이 확장의 제공자들은 이번 폴링을 지난 폴링과 비교하지 않고, 응답 하나에는 현재 순위만 담겨 있어서 변동 라인을 정직하게 만들어낼 수 없습니다. 대신 폴링마다 갱신되는 선두가 경기 행에 표시되고, 중계에는 세션 시작·세션 종료·상위 3명 결과가 흐릅니다.
- **대학미식축구는 기본으로 꺼져 있습니다.** `espn:college-football`은 완전히 지원하지만 기본 리그 세트에는 없습니다 — 토요일 하루에 99경기 안팎이 잡히는데 트리는 경기당 한 행을 그리므로 다른 리그가 전부 묻히기 때문입니다. [대학미식축구 켜기](#대학미식축구-켜기)를 참고하세요.
- **테니스는 대회당 최대 12경기까지만 표시합니다.** 한 대회에서 드로 전체로 124경기가 내려온 적이 있어, 대회마다 가장 관련성 높은 12경기(진행 중 > 방금 종료 > 예정)만 남기고 몇 경기를 제외했는지는 로그에 남깁니다. 이 12자리는 대회의 드로에 라운드로빈으로, 단식 드로를 먼저 배분하므로 그랜드슬램에서 12자리가 전부 복식으로 채워지는 일은 없습니다.
...
Vibe Stealth는 MLB, NHL, ESPN, 네이버, 라이엇 게임즈/LoL Esports, PandaScore, ATP, WTA, 포뮬러 1, UFC, NCAA를 비롯한 그 어떤 리그·팀과도 **제휴·보증·후원 관계가 없습니다.** 모든 팀·리그 명칭과 로고는 각 권리자에게 있습니다.
